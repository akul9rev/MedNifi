...
app.post("/api/register", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Pharmacy and user registered successfully" });
  } catch (error) {
...
app.post("/api/medicines", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const { name, batch_number, stock_quantity, expiry_date, price_per_unit } =
      req.body;

    if (!name || !batch_number || !expiry_date) {
      return res
        .status(400)
        .json({ error: "Name, batch number and expiry date are required" });
    }
  ...
  try {
    ...
    await connection.query(
      ...
      [
        ...
        req.user.id,
        requestedQty,
        stockItem.price_per_unit * requestedQty,
      ],
    );
...
app.delete("/api/medicines/:stockId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await connection.query(
      "DELETE FROM stock WHERE id = ? AND pharmacy_id = ?",
      [stockId, req.user.pharmacy_id],
    );
    ...
    if (remainingStock.length === 0) {
      await connection.query(
        "DELETE FROM medicine WHERE id = ? AND pharmacy_id = ?",
        [medicineId, req.user.pharmacy_id],
      );
    }
...
app.get("/api/analytics", authenticateToken, async (req, res) => {
  try {
    ...
    const [revenue] = await db.query(
      "SELECT SUM(sale_price) as total_revenue FROM sale WHERE pharmacy_id = ?",
      [req.user.pharmacy_id],
    );

    // Total Stock
    const [stock] = await db.query(
      "SELECT SUM(quantity) as total_items FROM stock WHERE pharmacy_id = ?",
      [req.user.pharmacy_id],
    );
...
async function refreshAlertsForPharmacy(connection, pharmacyId) {
  await connection.query("DELETE FROM expiry_alert WHERE pharmacy_id = ?", [
    pharmacyId,
  ]);
  await connection.query("DELETE FROM stock_alert WHERE pharmacy_id = ?", [
    pharmacyId,
  ]);
